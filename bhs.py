...
class ClebschGordanCoefficients():
    ...
    def transform1(self):
        '''Transform: (i) <=> (iii) and (ii) <=> (iv)'''
        l1, l2, j1, j2 = self.l1, self.l2, self.j1, self.j2
        
        self.l1 = 1/2*(l1 + l2 - self.j)
        self.l2 = 1/2*(l1 + l2 + self.j)
        self.j1 = 1/2(-l1 + l2 + j1 - j2)
        self.j2 = 1/2*(-l1 + l2 - j1 + j2)
        self.j = l2 - l1
    ...
    def evaluate(self):
        qpoch = qPochhammer(1, expand=False)
        bhs   = BasicHypergeometricSeries(3,2,expand=False)
...
# Test values
l1, l2, l, q, j1, j2, j = var('l1 l2 l q j1 j2 j')
C = ClebschGordanCoefficients(q, l1, l2, l, j1, j2, j)
